...
namespace Geointegrasjon.Matrikkelfoering.ReceiveSample
{
    class Program
    {
        ...
        {   
            ...
            // NB! Det er maks 200 meldinger som kan hentes, må rydde og sette Lest en gang i blant (Sett forsendelse mottatt)
            ...
            if (responseList.Count > 0)
            {
                ...
                {
                    //++idx;
                    if (idx >= startIdx) // TODO: Fix this
                    {
                        var handled = await ProcessWaitingMessage(client, waitingMessage);
                        //if (handled)
                        //{
                        //    ++idx;
                        //}
        ...
        private static string  MakeUniqueEndingToTitle(string title)
        {
            // Check if message have been cancelled, tittel should end with "#number", e.g. #2.
            if (Regex.Match(title, @".*#\d+$").Success)
            {
                var ipos = title.LastIndexOf("#");
                var number = title.Substring(ipos + 1);
                int newNumber = ((int)Convert.ToInt32(number)) + 1;
                string newTitle = title.Substring(0, ipos + 1) + newNumber.ToString();
                return newTitle;
            }
            else
            {
                string newTitle = title + "_#2";
                return newTitle;
            }
        }
